...
class KSLGigWorkersAgent(BaseAgent):
    ...
    - Potentially under-employed and seeking more
    - Comfortable with commission / variable income
    """

    def __init__(self, config: dict[str, Any], db: Any) -> None:
        super().__init__(
            name="ksl_gig_workers",
            platform="ksl",
            config=config,
            db=db,
        )
    ...
    def parse_item(self, raw_data: dict) -> dict:
        """Convert raw KSL gig worker data into the standard lead format."""
