...
class KSLCoachingConsultingAgent(BaseAgent):
    ...
    - Entrepreneurial with existing client networks
    - Already in "helping others succeed" roles
    - May be seeking additional or alternative income streams
    """

    def __init__(self, config: dict[str, Any], db: Any) -> None:
        super().__init__(
            name="ksl_coaching_consulting",
            platform="ksl",
            config=config,
            db=db,
        )
    ...
    def parse_item(self, raw_data: dict) -> dict:
        """Convert raw KSL coaching/consulting data into the standard lead format."""
