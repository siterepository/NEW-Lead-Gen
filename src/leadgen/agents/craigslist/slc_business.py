...
SEARCH_PATH = "/search/bfa"  # business for sale / franchise
...
class CraigslistSLCBusinessAgent(BaseAgent):
    ...
    - Entrepreneurial with business management experience
    - Potentially seeking new income-producing opportunities
    - Experienced with sales, clients, and revenue generation
    """

    def __init__(self, config: dict[str, Any], db: Any) -> None:
        super().__init__(
            name="craigslist_slc_business",
            platform="craigslist",
            config=config,
            db=db,
        )
    ...
    def get_search_urls(self) -> list[str]:
        """Build Craigslist search URLs for SLC business-for-sale."""
        urls: list[str] = []
    ...
    def parse_item(self, raw_data: dict) -> dict:
        """Convert raw CL business-for-sale data into the standard lead format."""
