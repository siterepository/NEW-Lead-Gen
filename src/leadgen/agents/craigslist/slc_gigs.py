...
SEARCH_PATH = "/search/ggg"  # gigs section
...
class CraigslistSLCGigsAgent(BaseAgent):
    ...
    - Entrepreneurial and comfortable with variable pay
    - In the Salt Lake City metro area
    - Potentially open to commission-based careers
    """

    def __init__(self, config: dict[str, Any], db: Any) -> None:
        super().__init__(
            name="craigslist_slc_gigs",
            platform="craigslist",
            config=config,
            db=db,
        )
    ...
    def get_search_urls(self) -> list[str]:
        """Build Craigslist search URLs for SLC gigs."""
        urls: list[str] = []
    ...
    async def scrape(self) -> list[dict]:
        """Scrape Craigslist SLC gigs listings.
    ...
    def parse_item(self, raw_data: dict) -> dict:
        """Convert raw CL gigs data into the standard lead format."""
