...
class CraigslistProvoJobsWantedAgent(BaseAgent):
    ...
    def __init__(self, config: dict[str, Any], db: Any) -> None:
        super().__init__(
            name="craigslist_provo_jobs_wanted",
            platform="craigslist",
            config=config,
            db=db,
        )
    ...
    def get_search_urls(self) -> list[str]:
        """Build Craigslist search URLs for Provo jobs wanted."""
        urls: list[str] = []
    ...
    async def scrape(self) -> list[dict]:
        """Scrape Craigslist Provo jobs-wanted listings.
    ...
    def parse_item(self, raw_data: dict) -> dict:
        """Convert raw CL Provo jobs-wanted data into the standard lead format."""
